  ...
  export default {
    ...
    methods: {
      ...
      async beforeSendMsg(param = {},_continue = true) {
        ...
        // 检查chatInputContent是否存在
        if (!this.chatInputContent && !Object.keys(param).length) {
          return uni.showToast({
            title: '不能发送空消息',
            icon: 'none'
          });
          ...
          for (let key in param) {
            if (param[key]) {
              ...
              try {
                ...
                console.error('JSON parse error in param:', e)
                msg.body = param[key]
              }
            }
          }
        ...
        if(_continue){
          ...
          console.error('beforeSendMsg error:', e)
          return uni.showToast({
            title: '消息处理失败',
            icon: 'none'
          });
      ...
      sendMsg(msg, callback) {
        try {
          // 确保msg对象存在
          if (!msg) {
            console.error('sendMsg: msg is undefined')
            return;
          }
          ...
          try {
            ...
            if(tmpBody && tmpBody.includes('个推')){
              msg.body = JSON.parse(tmpBody.replace(/个推/g,'个\u200b推'))
            }
          } catch (e) {
            console.error('Process msg body error:', e)
          }
