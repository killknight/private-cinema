<template>
	<view class="admin-mgr-container">
		<!-- 搜索框 -->
				...
				<view class="user-info">
					...
					<view class="user-details">
						...
						<view class="user-role">
							{{ user.isAdmin ? '当前身份：管理员' : '当前身份：普通用户' }}
						</view>
					</view>
				</view>
				<view class="action-buttons">
					<button v-if="!user.isAdmin" class="set-admin-btn" @click="setAdmin(user._id)">
						设置管理员
					</button>
					<button v-else class="cancel-admin-btn" @click="cancelAdmin(user._id)">
						取消管理员
					</button>
				</view>
			</view>
						...
						if (res.confirm) {
							...
							uniCloud.callFunction({
								...
								if (res.result && res.result.code === 0) {
									...
									const userIndex = this.users.findIndex(u => u._id === userId);
									if (userIndex !== -1) {
										this.users[userIndex].isAdmin = false;
									}
									uni.showToast({
										title: '取消成功'
									});
